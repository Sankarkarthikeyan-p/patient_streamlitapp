...
def create_database(db):
    """Create the 'patientdb' database if it doesn't exist."""
    cursor = db.cursor()
...
def create_appointment_table(db):
    """Create the appointment table in the database."""
    cursor = db.cursor()
    ...
    db.commit()
    st.write("appointment table created successfully.")
...
def insert_appointment_record(db, patient_id, appointment_date, appointment_time, doctor_name, notes):
    ...
    INSERT INTO appointment (patient_id, appointment_date, appointment_time, doctor_name, notes)
    VALUES (%s, %s, %s, %s, %s)
    """
...
def show_all_appointment(db):
    cursor = db.cursor()

    # Select the database
    cursor.execute("USE patientdb")
    select_query = """
    SELECT id, patient_id, appointment_date, CAST(appointment_time AS CHAR), doctor_name, notes FROM appointment
    """
...
def fetch_appointment_by_id(db, appointment_id):
    """Fetch an appointment's record from the 'appointment' table based on ID."""
    cursor = db.cursor()
...
def main():
    ...
    #config['database'] = 'patientdb'  # Update the database name
    #db = create_connection()

    #create_patients_table(db)
    #create_appointment_table(db)
    #modify_patients_table(db)

    menu = ["Home","Add patient Record","Show patiet Records", "Search and Edit Patient","Deetel Patients Record",
            "Add patients appointment","Show All appointment","Search and Edit Patients appointment"]
    ...
    elif options == "Add patients appointment":
    ...
    elif options=="Show All appointment":
         show_all_appointment(db)


    elif options == "Search and Edit Patients appointment":
